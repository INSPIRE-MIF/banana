  ...
  function (angular, app, _, d3, localRequire) {
    ...
    module.controller('spatial', function ($scope, $timeout,
      ...
      function computeLegend() {
        ...
        for (var i = 0; i < $scope.panel.colors.length; i++) {
          ...
          if ($scope.domain.length > 0) {
            var color = $scope.scale(classeMin);
            if (color) {
              $scope.legend.push({
                id: i,
                // Color array may contains extra quote. FIXME
                color: color ? color.replace(/"/g, '') : '#FFF',
      ...
      $scope.addThematicLayer = function () {
        ...
        var displayFeatureInfo = function(pixel) {
          ...
          if (feature) {
            ...
            if (value) {
              ...
              info.css({
                left: (pixel[0] + mapPos.left + 10) + 'px',
        ...
        map.on('singleclick', function(evt) {
          var feature = map.forEachFeatureAtPixel(evt.pixel,
            function(feature, layer) {
              var filterId = filterSrv.idsByTypeAndField('querystring', $scope.panel.thematicField, true);
              filterSrv.set({
                type: 'querystring',
        ...
        //   layers:[$scope.thematicLayer]  //Setting layers to be hovered
        // });
      ...
      $scope.getData = function () {
        ...
        results.then(function(results) {
          if ($scope.panel.mapMode === 'heatmap') {
            ...
            var data = results.facets && results.facets.buckets && results.facets.buckets.buckets;
            if (data) {
